...
and LIFT interaction features are downgraded; LIFT-Stack is the strongest
low-novelty control.

| Rank | Candidate | Mechanism | Fair-test status | Compute | Risk | Status |
|---:|---|---|---|---:|---|---|
| 1 | LIFT-Stack | LIFT-gated multi-hop fixed feature stack | improves Cora/CiteSeer/Chameleon/Squirrel and protects WebKB/Actor/Cornell, but overlaps SIGN/SGC/FAF/PROPGCL | done | HIGH novelty risk | STRONG_BASELINE_LOW_NOVELTY |
| 2 | LIFT-HC-GCL | hop-drop contrastive encoder on top of LIFT-Stack | Cora/Wisconsin local gains, but CiteSeer/Chameleon regress; transformation branch unstable | done | MEDIUM-HIGH | FAILED_MIXED_SPLIT0_GATE |
| 3 | LIFT-Stack + checkpoint residual | concat existing BGRL/GDC/TD checkpoint embedding with LIFT-Stack | Cora improves locally, but CiteSeer/Chameleon/WebKB regress | done | MEDIUM | FAILED_COMPLEMENTARITY_GATE |
| 4 | LIFT Channel Gate | channel-wise edge-lift weights on LIFT-Stack | Chameleon split-0 improves, but Chameleon 10-split is neutral and Squirrel can regress | done | MEDIUM | FAILED_ROBUSTNESS_GATE |
| 5 | LIFT interaction features | fixed raw-propagation product/difference/delta blocks | tiny Cora/CiteSeer local gains, but Chameleon fails | done | MEDIUM | FAILED_SPLIT0_GATE |
| 6 | LIFT-PROP-GCL | label-free edge-lift abstention plus K1/K2 selector | selector v1 hits oracle on 7/8 observed settings, but R077/R078 failed to make it a full GCL-training method | done | MEDIUM | SELECTOR_BASELINE_NOT_MAIN_METHOD |
| 7 | GDC-GCL+ | gradient-residual training dynamics positives/weights | split-0 gate failed | done | MEDIUM | FAILED_SPLIT0_GATE |
