...
**Current lead**: Cycle-Balance Gated LIFT
...
Sharpness-Stable Alignment is weak-internal-only; LIFT-Portfolio remains the
strongest low-novelty control; Cycle-Balance Gated LIFT is the current active
candidate with high novelty risk.

| Rank | Candidate | Mechanism | Fair-test status | Compute | Risk | Status |
|---:|---|---|---|---:|---|---|
| 1 | Cycle-Balance Gated LIFT | feature-induced signed edges plus balanced/unbalanced signed path channels, activated only under LIFT-Portfolio `single_k2_low_raw_lift` gate | Chameleon 10-split full-grid improves `0.685526 -> 0.725877`; Cora/CiteSeer/WebKB and Squirrel/Actor/Cornell are protected in current checks | active proxy | HIGH signed-GCL/heterophily-GCL novelty risk | ACTIVE_CANDIDATE_WITH_NOVELTY_RISK |
...
| 3 | LIFT-Stack | LIFT-gated multi-hop fixed feature stack | improves Cora/CiteSeer/Squirrel and fast-grid Chameleon, but full-grid Chameleon prefers P2; overlaps SIGN/SGC/FAF/PROPGCL | done | HIGH novelty risk | COMPONENT_BASELINE |
...
Cycle-Balance Gated LIFT is now the active candidate. It uses the
LIFT-Portfolio gate to decide when to append feature-induced signed-path
channels. The full-grid Chameleon 10-split result improves from portfolio
`0.685526 +/- 0.021292` to `0.725877 +/- 0.019174`, while current split-0
checks protect Cora/CiteSeer/Texas/Wisconsin/Squirrel/Actor/Cornell unchanged.
Novelty remains cautious because signed graph contrastive learning and
heterophily graph contrastive learning are close priors. Next required step is
full ablation and sign-shuffle controls, not paper writing.
