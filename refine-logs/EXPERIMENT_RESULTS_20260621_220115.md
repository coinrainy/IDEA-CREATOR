...
暂停 10 split 扩展和外部强基线主表。优先回到方法层面做缺陷修订：SRLP 当前 residual target 可能过难或与分类语义弱相关；可以先审查 residual target 的 label alignment、target norm 分布、projection rank，以及是否需要从“完全 residual 预测”降级为“full latent + residual auxiliary”的更保守目标。
