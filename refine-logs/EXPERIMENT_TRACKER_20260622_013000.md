...
## 2026-06-22 Adaptive Gate 补充追踪

| Run ID | 模块 | 目的 | 系统 / 变体 | 数据集 / Split | 指标 | 优先级 | 状态 | 备注 |
|---|---|---|---|---|---|---|---|---|
| R018 | 方法诊断 | label-free gate 是否能区分 residual 有用性 | SRLP-Aux checkpoints | Chameleon/Texas/Wisconsin/Actor split 0-9 | g_r/g_p/rho/stability | MUST | DONE | residual gate 能抬高 Texas/Wisconsin，但 Chameleon projection gate 绝对值偏低；只允许极小 pilot。 |
| R019 | 实现 smoke | 新增自适应目标 | `srlp_adaptive_aux` | Chameleon split 0, 5 epochs | JSON/CSV + NaN/collapse | MUST | DONE | 运行通过，无 NaN/collapse；Chameleon 约 2.5 秒/epoch。 |
| R020 | 极小 pilot | 自适应残差辅助是否转化为表征收益 | `srlp_adaptive_aux` | Chameleon/Texas/Wisconsin split 0, 200 epochs | valid/test + gates | MUST | DONE | Wisconsin `test@best=0.6078`，比 best non-adaptive 高 0.0196；Chameleon/Texas 均低于最强对照。 |
| R021 | 公平对照 | 避免 200 epoch 与 1000 epoch 误比 | FullLatent/ZPZ/SRLP-Aux | Chameleon/Texas/Wisconsin split 0, 200 epochs | valid/test + diagnostics | MUST | DONE | 用于 R020 对照；结论是不扩展 Adaptive-Aux。 |

当前决策：停止 SRLP/Adaptive-Aux 作为主方法扩展，不进入外部强基线、10 split 扩展或大图实验。若继续该线，应改为分析型诊断问题，而非继续堆叠 SRLP 变体。
