...
Teacher branch, clean graph:
  Z_bar = f_bar(X, A)
  For each target v:
    z_t(v) = stopgrad(Z_bar(v))
    z_c(u) = stopgrad(normalize(Z_bar(u))), u in N_ctx(v)
    q_v = normalize(mean(z_c(u)))
    R_raw(v) = z_t(v) - q_v (q_v^T z_t(v))
    R_bar(v) = R_raw(v) / (||R_raw(v)||_2 + epsilon)
...
Online branch, target-isolated context graph:
  ...
  c(v) = normalize(mean_{u in N_ctx(v)} normalize(H_ctx(u)))
  P_v = normalize(g_theta(c(v)))

Loss:
  L = mean_{v in T_valid} [1 - cosine(P_v, R_bar(v))]
  Optional fallback only if collapse appears: L = L + beta L_var
```
...
z_t(v) = stopgrad(Z_bar(v))
z_c(u) = stopgrad(normalize(Z_bar(u))), u in N_ctx(v)
q_v = normalize(mean_{u in N_ctx(v)} z_c(u))
R_raw(v) = z_t(v) - q_v (q_v^T z_t(v))
T_valid = {v in T | |N_ctx(v)| > 0 and ||R_raw(v)||_2 > epsilon}
R_bar(v) = R_raw(v) / (||R_raw(v)||_2 + epsilon), v in T_valid
```

If `N_ctx(v)` is empty or `||R_raw(v)||_2 <= epsilon`, skip `v` for this batch and record:

```text
skipped_ratio = 1 - |T_valid| / |T|
```

The pilot uses rank-1 only. Rank-k QR projection is not part of v1.
...
```text
L_pred = mean_{v in T_valid} [1 - cosine(P_v, R_bar(v))]
L = L_pred
```

`R_bar(v)` is already stop-gradient because it is constructed only from stop-gradient teacher latents. No conditional InfoNCE, no covariance regularizer, and no teacher-side regularization in v1.

Use the following only as a collapse fallback, not as part of the main method:

```text
L_var = mean_j max(0, gamma - std(P_T_valid[:, j]))
L = L_pred + beta * L_var, only if collapse is observed
```
...
4. Monitor `||R_raw(v)||`, `skipped_ratio`, prediction cosine, and embedding rank.
5. Skip low-energy targets with `epsilon`, but lower `epsilon` or reduce target ratio if `skipped_ratio` becomes high.
6. Optional full-latent warmup may be used only as stabilization; if used, report no-warmup ablation.
...
- implement `T_valid`, `epsilon`, and `skipped_ratio` logging;
